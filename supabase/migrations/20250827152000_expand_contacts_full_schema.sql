...
alter table public.contacts
  ...
  add column if not exists how_we_met text check (how_we_met in ('job_application','networking_event','referral','linkedin','conference','other')),
  add column if not exists relationship_strength text not null default 'cold' check (relationship_strength in ('cold','warm','strong','advocate')),
