...
export const EditInterviewFeedbackDialog = ({ feedback, onSubmit, open, onOpenChange }: Props) => {
  const { jobs = [] } = useJobs();
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      interview_time: feedback.interview_time || "",
      interview_type: feedback.interview_type,
      interview_round: feedback.interview_round,
      interview_format: feedback.interview_format,
      duration_minutes: feedback.duration_minutes,
      location_platform: feedback.location_platform || "",
      interviewers: feedback.interviewers.length > 0 ? feedback.interviewers : [{ name: "", title: "", email: "" }],
      questions_answers: feedback.questions_answers.length > 0 ? feedback.questions_answers : [{ question: "", answer: "", notes: "" }],
      technical_assessment: feedback.technical_assessment,
      salary_discussed: feedback.salary_discussed,
  ...
  const handleSubmit = (values: z.infer<typeof formSchema>) => {
    const updatedFeedback: InterviewFeedback = {
      ...feedback,
                  ...
                  <Select 
                    onValueChange={(value) => {
                      ...
                      if (value && value !== "manual") {
                        ...
                        // Don't clear company/position on manual selection for edit mode
                      }
                    }} 
