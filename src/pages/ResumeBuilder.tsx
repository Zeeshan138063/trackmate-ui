...
interface ResumeBuilderProps {
  initialTab?: "resumes" | "templates" | "builder";
  showTabs?: boolean;
}

export default function ResumeBuilder({ initialTab = "resumes", showTabs = true }: ResumeBuilderProps) {
  const [activeTab, setActiveTab] = useState(showTabs ? initialTab : "builder");
  ...
  const [summary, setSummary] = useState("Experienced software engineer with expertise in full-stack development...");
  const [skills, setSkills] = useState<string[]>(["React", "TypeScript", "Node.js", "Python", "AWS"]);
  const [newSkill, setNewSkill] = useState("");
  const [experiences, setExperiences] = useState<Experience[]>([
    { id: "1", title: "Software Engineer", company: "Tech Company Inc.", startDate: "2022-01-01", endDate: "2024-12-31", description: "Developed and maintained web applications using React and Node.js..." }
  ]);

  const addExperience = () => {
    setExperiences((prev) => [
      ...prev,
      { id: String(Date.now()), title: "", company: "", startDate: "", endDate: "", description: "" }
    ]);
  };
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input type="email" placeholder="john.doe@email.com" value={email} onChange={(e) => setEmail(e.target.value)} />
                  </div>
                  <div>
                  ...
                  <div>
                    ...
                    <Input placeholder="Software Engineer" value={headline} onChange={(e) => setHeadline(e.target.value)} />
                  </div>
                </CardContent>
              </Card>
                    ...
                    <div key={exp.id} className="p-4 border rounded-lg space-y-4">
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        <div>
                          ...
                          <Input placeholder="Tech Company Inc." value={exp.company} onChange={(e) => updateExperience(exp.id, { company: e.target.value })} />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                      ...
                      <div>
                        ...
                        <Textarea rows={3} placeholder="Describe your responsibilities and achievements..." value={exp.description} onChange={(e) => updateExperience(exp.id, { description: e.target.value })} />
                      </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-3">
                  <div className="flex gap-2">
                    <Input placeholder="e.g. LangChain" value={newSkill} onChange={(e) => setNewSkill(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); handleAddSkill(); } }} />
                    <Button type="button" onClick={handleAddSkill}>Add</Button>
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {skills.map((s, i) => (
                      <Badge key={`${s}-${i}`} variant="secondary" className="cursor-pointer" onClick={() => handleRemoveSkill(i)} title="Click to remove">{s} ×</Badge>
                    ))}
                  </div>
                </CardContent>
              </Card>
            ...
            <div className="lg:sticky lg:top-6">
              <Card>
                ...
                <CardContent>
                  <div id="print-resume" ref={previewRef} className="aspect-[8.5/11] bg-white border rounded-lg p-6 text-black">
                    <div className="text-center mb-6">
                      <h1 className="text-2xl font-bold">{fullName || "Your Name"}</h1>
                      <p className="text-gray-600">{headline || "Role / Headline"}</p>
                      <p className="text-sm text-gray-500">{email} | {phone} | {location}</p>
                    </div>
                    ...
                    <div className="mb-6">
                      ...
                      {experiences.map((exp) => (
                        <div key={exp.id} className="mb-3">
                          ...
                          <p className="text-sm text-gray-600">{exp.company || "Company"} | {exp.startDate || "Start"} - {exp.endDate || "Present"}</p>
